...
\textbf{1{,}312 named theorems across 68 files}, covering core QUIC algorithms
...
All 1{,}312 theorems are fully sorry-free; 0~\texttt{sorry} obligations remain.
Proofs are verified continuously by CI.
Behavioural correspondence between the Lean functional models and the Rust
source is validated by 26 Route-B test suites covering 2{,}822+ cases, all passing.
...
(4)~a formal characterisation of the HTTP/3 settings RFC-compliance
    invariants, with 26 Route-B test suites (2{,}822+ cases, all PASS)
...
\begin{itemize}
  ...
  \item \textbf{1{,}312 Lean~4 theorems} (all sorry-free) across 68 algorithmic
    components, covering QUIC's byte framing, congestion control (including
    BBR2 bandwidth arithmetic, BBR2 Limits invariants, BBR2 phase constants
    (Startup/Drain/ProbeRTT/CyclePhase), and the gcongestion
    ...
    QPACK integer encoding, packet-type/epoch mapping, and path-validation state.
  \item \textbf{26 Route-B correspondence test suites} covering 2{,}822+ cases
...
Of 68 files audited, \textbf{no mismatches} were found.
...
Table~\ref{tab:theorems} summarises the 68 Lean files.
The project totals \textbf{1{,}312 named theorems}, \textbf{0 \texttt{sorry}},
verified by Lean~4.29.0 via the Lake build system.
Correspondence is validated by \textbf{26 Route-B test suites} (2{,}822+ cases, all PASS).
...
\midrule
\multicolumn{3}{l}{\textbf{Total}}                                               & \textbf{1{,}312} & \textbf{0 sorry} & \\
\bottomrule
\end{tabular}
\caption{Summary of 68 Lean~4 files (representative selection shown): Rust source, logical layer, named
...
The 68 files form five logical layers (Framing, Algorithm, Congestion,
Stream, H3/QPACK), as shown in Figure~\ref{fig:arch}.
The Algorithm layer has grown from 9 to 20+ files, the Congestion layer from 5 to 15+,
...
This is itself a positive finding: the 1{,}312 theorems provide formal
...
\texttt{CORRESPONDENCE.md} and validated by 26 Route-B test suites
covering 2{,}822+ cases across 26 representative targets, all PASS.
...
We have presented an incremental formal verification of 68 algorithmic
components in \texttt{quiche}, Cloudflare's production QUIC implementation,
using Lean~4 without Mathlib.
The project has accumulated \textbf{1{,}312 machine-checked theorems}, all
fully sorry-free.
The proofs cover byte framing, congestion control (including BBR2 bandwidth
arithmetic, BBR2 Limits struct, BBR2 phase constants (Startup/Drain/ProbeRTT/CyclePhase),
gcongestion pacing-rate invariants, and
...
bounds, path-validation state, packet-type/epoch mapping, and HTTP/3/QPACK
(frame codec, settings RFC-compliance, QPACK static table, QPACK integer codec).
Correspondence between Lean models and Rust source is validated by
\textbf{26 Route-B test suites covering 2{,}822+ cases, all PASS}.
...
Future work includes: verifying CUBIC cubic\_k/wmax invariants; modelling
the QUIC idle-timeout state machine; and exploring Aeneas extraction to
mechanically bridge the Lean model to the Rust source.
